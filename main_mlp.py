...
model = SimpleMLP(input_dim, hidden_dim, output_dim, scale, rank = args.init_rank).to(device)
...
for epoch in range(num_epochs):
    ...
    if epoch == 0:
        ...
        emp_ntk_pre_log = emp_ntk

    emp_ntk_change = torch.norm(emp_ntk - init_emp_ntk, p = 'fro').item() / torch.norm(init_emp_ntk, p = 'fro').item()
    emp_ntks.append(emp_ntk_change)
    change_rate = torch.norm(emp_ntk - emp_ntk_pre_log, p = 'fro').item() / torch.norm(emp_ntk_pre_log, p = 'fro').item()
    emp_ntks_change_rate.append(change_rate)
    ...
    ax2 = ax1.twinx()
    color = 'tab:red'
    ax2.set_ylabel('emprical NTK relative change (t + 1 v.s. t)', color=color)
    ax2.plot(emp_ntks_change_rate, label='emprical NTK relative change', color=color)
    ax2.tick_params(axis='y', labelcolor=color)
    ...
    plt.title('Accuracy and emprical NTK relative change vs Epochs')
    plt.savefig(f"results_mlp/emprical_ntk_relative_change_{args.label}.png")
    plt.close()
