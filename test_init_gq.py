...
# test if grokfast leads the model to a flatter region.
# Set a fixed random seed
torch.manual_seed(42)
rank = 1

# Initialize two identical MLPs
model1 = SimpleMLP(input_dim = input_dim, hidden_dim = hidden_dim, output_dim = output_dim, scale = 4.0, rank = None)
model2 = SimpleMLP(input_dim = input_dim, hidden_dim = hidden_dim, output_dim = output_dim, scale = scale, rank = 1)
models = [model1, model2]
labels = ['small init', 'rank 1']
filters = ['none', 'none']

# metainit_with_dataset(model2, criterion, train_loader, device, lr=0.1, momentum=0.9, steps=100, eps=1e-5)
